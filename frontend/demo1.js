     ...
     function bk_getall()
     {
      ...
      fres.then(function (response) {
          ...
          function bk_da(data)
          {
             table="<table align=center><tr><th>book_NAME</th><th>book_NAME</th><th>cost</th><th>auth</th><th>pub</th><th>des</th></tr>";
             for (var i = 0; i < data.length; i++) {
                 table+="<tr><td>"+"<div class='card'><img src='./Images/"+data[i].name+".png' alt='no img'></div>"+"</td><td>"+data[i].name+"</td><td>"+data[i].cost+"</td><td>"+data[i].author+"</td><td>"+data[i].publisher+"</td><td>"+data[i].description+"</td></tr>";
...
function bk_delete()
{
  ...
  function pym()
  {
      ...
      document.getElementById("tcdata").innerHTML=bookstore;
      }
